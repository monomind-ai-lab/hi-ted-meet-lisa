...
| `languages` | [script] on `monomind-deck`; [agent] everywhere else | Trim or extend the language switch to exactly this set. English always stays — and when English is *all* that is left, delete the switch, the `#google_translate_element` div and the translate script entirely, not just the surplus buttons; a one-language switch is a control that does nothing. The script does all of this on the MonoMind deck. On every other template each language is written inline, so trimming or adding one is content work — the script reports it `NOT-MECHANICAL`. Never infer a language from the subject: a deck *about* a Taiwanese client is not a request for Chinese. Only this answer decides. **Check the built file before handover** — `python3 scripts/check_languages.py FILE --languages <this answer>` drives the file in a browser and fails when a chosen language is unreachable or an unchosen one is still offered. On the nine templates where this row is `[agent]`, that check is the only thing standing between the answer and the file. |
...
| `credit: true` or absent | [script] | Keep the colophon — the "Made with Hi Ted, Meet Lisa" line every template carries in its footer or closing slide, linking to www.hitedmeetlisa.cc. Each pattern reference shows the exact markup. |
...
| `share: true` or absent | [script] | Keep the Share control — the small menu in the template's own chrome whose one action, **Publish link**, hands the file to htmlbyme.com for a public link that lives 15 days. Every first-party template ships it, each in its own idiom. Nothing is sent anywhere until a reader clicks it and then confirms on htmlbyme.com. |
| `share: false` | [script] | Delete both `LISA:SHARE-START` / `LISA:SHARE-END` regions — the control's stylesheet in `<head>` and its markup + script in the chrome — and the sentence in the `LISA:CONTENT-MAP` header that names them. Nothing outside those fences refers to anything inside them, so the file is what it was before the control existed: no markup, no styles, no listeners, no strings. This is the answer a corporate or brand pack sets. |
