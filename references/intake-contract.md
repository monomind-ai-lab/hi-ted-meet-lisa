...
| `credit` | `true` \| `false` | Whether the file keeps its colophon — the "Made with Hi Ted, Meet Lisa" line linking to www.hitedmeetlisa.cc. Every template ships it; `false` means delete that one line (never the logo or identity links, which belong to `logo`). Asked for **every** shape, the external handoff included, so it is present in both the runner's payload and the web panel's paste-ready prompt alike. |
| `share` | `true` \| `false` | Whether the file keeps its **Share** control — a small menu in the template's own chrome whose one action, **Publish link**, hands the finished file to [htmlbyme.com](https://htmlbyme.com), a separate free MonoMind service that hosts one HTML file behind a public link for 15 days. Asked for **every** shape, the external handoff included, exactly like `credit`, and defaulted to `true` the same way. `false` deletes the control, its styles and its script — the two `LISA:SHARE-START`/`LISA:SHARE-END` regions — leaving no markup, no listeners and no strings behind; `scripts/tedandlisa_apply.py` does it mechanically. Nothing is sent anywhere until a reader clicks Publish link **and** then confirms on htmlbyme.com; the file never calls that service's API, and a file with `share: false` cannot reach it at all. Corporate and brand packs can ship with `share: false`. |
...
`export`, `credit`, `share`, `review`). That split is the same sort of thing as a
