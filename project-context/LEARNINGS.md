...
| --- | --- | --- |
| [`L-001`](#l-001-google-translate-rewrites-product-names-and-filenames) | Google Translate rewrites product names and filenames | Any deck or page that ships the language switch. |
| [`L-002`](#l-002-cjk-output-runs-flush-against-protected-spans) | CJK output runs flush against protected spans | Translated rendering of any deck using `.nt-term`. |
| [`L-003`](#l-003-the-browser-pane-misreports-fixed-chrome-on-this-deck) | The browser pane misreports fixed chrome on this deck | Verifying any horizontally scroll-snapped layout in an emulated viewport. |
| [`L-004`](#l-004-google-translate-applies-slowly-and-only-while-compositing) | Google Translate applies slowly and only while compositing | Verifying the language switch in any automated browser surface. |
| [`L-005`](#l-005-maintenance-instructions-need-observable-triggers-and-a-check) | Maintenance instructions need observable triggers and a check | Any instruction that asks an agent to maintain a document alongside its real work. |
| [`L-006`](#l-006-contrast-has-to-be-measured-by-painting-not-by-parsing) | Contrast has to be measured by painting, not by parsing | Any automated accessibility or contrast check in a browser. |
| [`L-007`](#l-007-a-theme-that-flips-text-must-flip-its-surfaces-and-generated-content-defeats-both) | A theme that flips text must flip its surfaces, and generated content defeats both | Any template offering a light/dark toggle. |
| [`L-008`](#l-008-var-does-not-resolve-inside-an-svg-presentation-attribute) | `var()` does not resolve inside an SVG presentation attribute | Any SVG themed with CSS custom properties. |
| [`L-009`](#l-009-headless-chrome-writes-the-screenshot-and-may-then-hang) | Headless Chrome writes the screenshot and may then hang | Any scripted screenshot capture. |
| [`L-010`](#l-010-an-unrecognised-flag-can-run-an-interactive-installer-at-its-defaults) | An unrecognised flag can run an interactive installer at its defaults | Any first-time invocation of an unfamiliar CLI installer. |
| [`L-011`](#l-011-anonymisation-must-be-case-insensitive-and-verified-by-grep) | Anonymisation must be case-insensitive and verified by grep | Any normalisation of real material before publishing it. |
| [`L-012`](#l-012-verifying-through-the-function-hides-a-dead-interface) | Verifying through the function hides a dead interface | Any UI verified from a console or a test harness rather than by using it. |
| [`L-013`](#l-013-an-embedded-browser-turns-target-blank-into-a-same-tab-navigation) | An embedded browser turns `target="_blank"` into a same-tab navigation | Any link offered from a page that holds unsaved state, previewed in an in-app browser. |
| [`L-014`](#l-014-measure-what-a-dependency-does-before-replacing-it) | Measure what a dependency does before replacing it | Any migration away from a third-party library. |
| [`L-015`](#l-015-gradient-text-clips-its-own-descenders) | Gradient text clips its own descenders | Any heading using `background-clip: text`, which is most gradient display type. |
| [`L-016`](#l-016-the-web-document-nav-has-no-narrow-breakpoint-and-the-burger-is-what-falls-off) | The `web-document` nav has no narrow breakpoint, and the burger is what falls off | `assets/tedandlisa-template-web-document.html`, and any deck generated from it. |
| [`L-017`](#l-017-a-themed-value-read-straight-after-the-toggle-is-the-transition-not-the-theme) | A themed value read straight after the toggle is the transition, not the theme | Any automated contrast or colour check run against a template with a theme control. |
| [`L-018`](#l-018-swapping-the-inline-language-pair-touches-five-coupled-places-none-of-them-the-content) | Swapping the inline language pair touches five coupled places, none of them the content | `web-document` and any template whose second language is written inline. |
| [`L-019`](#l-019-documentopen-reuses-the-frames-window-so-a-re-mounted-script-redeclares-its-own-consts) | `document.open()` reuses the frame's Window, so a re-mounted script redeclares its own consts | Any template that assembles a document at runtime and writes it into an iframe — currently `sitemap-ia`, and anything that copies its mount script. |
| [`L-020`](#l-020-a-class-rule-outranks-hidden-so-a-styled-panel-paints-over-a-working-page) | A class rule outranks `[hidden]`, so a styled panel paints over a working page | Any component toggled with the `hidden` attribute that also carries a class-level `display`. |
| [`L-021`](#l-021-extracting-a-template-leaves-registers-pointing-at-the-sources-shape) | Extracting a template leaves registers pointing at the source's shape | `/tedandlisa-new-template`, and any extraction from a data-driven document. |
| [`L-022`](#l-022-reveal-on-scroll-renders-a-blank-page-wherever-intersectionobserver-never-fires) | Reveal-on-scroll renders a blank page wherever IntersectionObserver never fires | Any template using the `.reveal` / `IntersectionObserver` pattern — `project-website` today, and any extraction that inherits it. |
| [`L-023`](#l-023-the-browser-pane-serves-stale-css-and-defers-style-recalc-so-palettes-must-be-measured-off-browser) | The browser pane serves stale CSS and defers style recalc, so palettes must be measured off-browser | Any contrast or token verification done through the preview pane. |
| [`L-024`](#l-024-a-capped-nav-row-has-no-wider-screen-to-grow-into-so-it-fails-by-wrapping) | A capped nav row has no wider screen to grow into, so it fails by wrapping | Any template whose nav sits inside a `max-width` container — `project-website`, and the same shape in `web-document`. |
| [`L-025`](#l-025-a-check-that-resolves-the-repository-from-the-cwd-goes-silent-when-the-harness-opens-a-parent-directory) | A check that resolves the repository from the cwd goes silent when the harness opens a parent directory | Any hook or script that locates a repository root by walking up from `CLAUDE_PROJECT_DIR`, a hook `cwd`, or `getcwd()`. |
<!-- context-index:end -->
...
## L-025: A check that resolves the repository from the cwd goes silent when the harness opens a parent directory

- Status: `accepted`
- Scope: Any hook or script that locates a repository root by walking up from
  `CLAUDE_PROJECT_DIR`, a hook `cwd`, or `getcwd()`.
- Learning: `context_triggers.py` searched those three candidates and their
  parents. Opening the workspace folder *above* the repository put every
  candidate outside it, so running the check printed
  `no project-context/ in .../hitedmeetlisa` and exited 0 — from a script
  sitting inside a repository that has one. The failure mode is the problem: a
  clean exit with no output makes a session where the check could not find the
  repository indistinguishable from a session where no trigger was open. The
  same launch also left `CLAUDE.md` and `.claude/settings.json` unloaded, so
  nothing else raised a hand either.
- Action: Give the resolver a last-resort fallback to its own install root
  (`Path(__file__).resolve().parents[3]`), ordered after the cwd candidates so a
  real project directory still wins. Generally: a check that can silently find
  nothing needs a fallback that cannot, otherwise it reports health it never
  verified.
- Evidence: [`../.agents/skills/project-context/scripts/context_triggers.py`](../.agents/skills/project-context/scripts/context_triggers.py), [`../.claude/settings.json`](../.claude/settings.json)
