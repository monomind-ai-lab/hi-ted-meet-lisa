...
| Project context | Trigger-driven on scaffold `0.3.0` (current upstream); doctor reports healthy. Reachable three ways now: the `CLAUDE.md` block, the session hooks, and a `/project-context` pointer under `.claude/skills/` — it previously had only the first two, and both go quiet when the harness opens a directory above the repository (`L-025`). The trigger check falls back to its own install root, so it resolves this repository from any cwd. Both registries open with a generated index (`D-021`). | [`SKILL.md`](SKILL.md), [`../.claude/skills/project-context/SKILL.md`](../.claude/skills/project-context/SKILL.md), [`../scripts/context_index.py`](../scripts/context_index.py) |

## Active work

| Initiative | Status | Next action | Evidence |
| --- | --- | --- | --- |
| Public site for `html.monomind.one` | `in-progress` | `site/` holds a dark-cinematic landing page (visual language from the Canva brand deck: blurred-poppy atmosphere, white frame device, cream ink) with hero, a three-path chooser for new users (try once / install / both, tabbed with hash deep-links), the seven-card gallery of real previews, and a closing Ted-and-Lisa scene. The intake panel gained a **web mode** (`CTX.mode === "web"`, injected only by `site/sync.sh`): the end state reads "Prompt is ready.", the payload is one complete paste-ready prompt (preamble + answers), and the copy names the agents it works with — runner and `file://` behaviour untouched. Figure art: `assets/ted-figure.png` / `lisa-figure.png` are grayscale luminance figures extracted from the Canva SVG export; the `-cream` variants are pre-tinted RGBA renders the site uses. `site/sync.sh` assembles the deployable folder from canonical artifacts at deploy time; copies are gitignored. The page carries a searchable language dropdown (English default; Korean and Traditional Chinese preferred; ~40 more) driven by the same cookie-based Google Translate machinery as the deck template, with the same term-protection pass. The topbar mark is a deploy-derived solid-white copy of the currentColor SVG, and both the mark and the footer's "MonoMind AI Lab" link to monomind.one. The gallery ends in a "Your Template" card (more on the way + install to add your own), and the medium section is figure-free with a `deck.html` spec card. `README.md` now opens with the website as the no-terminal path. Verified over `http://` end to end, including Korean translation with protected terms. Deployed with wrangler to the `hi-ted-meet-lisa` Pages project (live at hi-ted-meet-lisa.pages.dev). Auto-deploy is wired: `.github/workflows/deploy-pages.yml` runs `site/sync.sh` and ships on every push to main — it needs the `CLOUDFLARE_API_TOKEN` repository secret (Account · Cloudflare Pages · Edit) before it goes green; until then, deploy manually with `bash site/sync.sh && wrangler pages deploy site --project-name hi-ted-meet-lisa`. The brand name never translates: the hero `h1` is a notranslate block and the document title is pinned by a MutationObserver. SEO/LLMO: canonical + full OG/Twitter meta with a 1200×630 cover image rendered from the Canva page-one export (`assets/tedlisa-cover-og.jpg`), JSON-LD SoftwareApplication, `site/llms.txt`, `site/robots.txt`, and a favicon derived at deploy time (white mark on the dark-olive tile). The html.monomind.one CNAME is configured and the domain is live. Translation on the custom domain needed three fixes the pages.dev origin never showed: the language choice lives in localStorage (Google's widget consumes and deletes the googtrans cookie after translating), every boot scrubs googtrans at all cookie scopes and seeds exactly one host-only cookie (the widget goes inert when duplicates are visible, and it re-adds a zone-wide `.monomind.one` copy after each run — pages.dev is immune only because the Public Suffix List blocks that), and a fallback bounces the hidden combo when text stays untranslated. The intake panel's own copy is shape-neutral too — the opening brief asks "What is this about?", and only genuinely slide-shaped questions still say "deck". The three paths are now three peers rather than two options plus a combination: 01 answer here then paste, 02 paste first and answer in your own browser, 03 make it yours (install). The old "do both" flow is gone, its install content moved from `#pane-install` to `#pane-both`, and the selector-keyed translation entries were retargeted to match — a structural move like this silently strands `T` entries, so the dead-selector sweep (`sels.filter(s => !document.querySelector(s))`) is the check that catches it. The closing scene is one composed PNG (`assets/ted-and-lisa-in-frame.png`, 55KB, alpha kept because it sits on the dark ground, lazy-loaded with intrinsic size set). It started as an 843KB SVG of four embedded PNGs; the PNG export is 84KB, and a lossless pass — dropping `caBX` (Content Credentials, 17KB), `iTXt` and `eXIf`, then re-deflating at level 9 — took it to 55KB, 93% under the SVG. Missing paths now return a real 404 (`site/404.html`, which Cloudflare Pages picks up by name) instead of 200 with the whole homepage. The brand line is protected twice over: the hero caption span carries `notranslate`, and `TERMS` gained a lowercase `monomind(?: ai lab)?` alternative because the regex is case-sensitive and the caption is lowercase. Three positional translation selectors have now silently retargeted after markup edits — `.hero-art .caption span:last-child` (the second span was deleted, so it overwrote the brand line), plus `.scene-copy p` and `.file-copy > p`, but `.kicker` is itself a `<p>` and comes first, so each overwrote its own kicker and left the real paragraph in English; both now carry `:not(.kicker)`, the guard the `.section-head` entries already used instead of the three-part `ted-figure-cream` + frame + `lisa-figure-cream` composition; `sync.sh` copies it and no longer ships the retired cream figures. The copy no longer calls the output a "deck" by default — that framing read as a slide-only product. "Deck" now appears only where it is literally true (the `monomind-deck` template) or inside a list that shows range; elsewhere the noun is the file, or the shape is named explicitly (slides, a document, a diagram, a site). English, Korean, and Traditional Chinese are now first-class: their copy is hand-written in a selector-keyed dictionary (`T` in `site/index.html`) and switched in place — instant, no reload, no cookies, no Google — with the dropdown split into an Instant tier and a Machine-translated tier; only the ~40 long-tail languages still go through the widget. Verified: ko and zh-TW apply instantly, survive reloads, and round-trip back to English. The widget's select-then-reload path on html.monomind.one still wants a re-check once Google's rate limit (tripped during debugging) lifts, but it now only affects the long tail. The custom domain html.monomind.one is attached to the project but `pending`: wrangler's OAuth offers no DNS scope, so the `html` → `hi-ted-meet-lisa.pages.dev` proxied CNAME in the monomind.one zone must be added in the dashboard, after which it activates on its own. Next: release zips for the claude.ai path, which the page marks "coming soon". | [`../site/index.html`](../site/index.html), [`../site/sync.sh`](../site/sync.sh), [`../assets/tedandlisa-intake.html`](../assets/tedandlisa-intake.html) |
