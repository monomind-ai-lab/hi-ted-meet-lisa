...
| --- | --- | --- | --- |
| Public site for `html.monomind.one` | `in-progress` | `site/` holds a dark-cinematic landing page (visual language from the Canva brand deck: blurred-poppy atmosphere, white frame device, cream ink) with hero, a three-path chooser for new users (try once / install / both, tabbed with hash deep-links), the seven-card gallery of real previews, and a closing Ted-and-Lisa scene. The intake panel gained a **web mode** (`CTX.mode === "web"`, injected only by `site/sync.sh`): the end state reads "Prompt is ready.", the payload is one complete paste-ready prompt (preamble + answers), and the copy names the agents it works with — runner and `file://` behaviour untouched. Figure art: `assets/ted-figure.png` / `lisa-figure.png` are grayscale luminance figures extracted from the Canva SVG export; the `-cream` variants are pre-tinted RGBA renders the site uses. `site/sync.sh` assembles the deployable folder from canonical artifacts at deploy time; copies are gitignored. Verified over `http://` end to end, including the skip-to-done web-mode flow. Next: deploy on Cloudflare Pages (build command `bash site/sync.sh`, output dir `site`), then release zips for the claude.ai path, which the page marks "coming soon". | [`../site/index.html`](../site/index.html), [`../site/sync.sh`](../site/sync.sh), [`../assets/tedandlisa-intake.html`](../assets/tedandlisa-intake.html) |
