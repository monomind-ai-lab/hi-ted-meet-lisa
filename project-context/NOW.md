...
| Templates | Six registered: `monomind-deck` (horizontal slides, Google Translate on demand), `web-document` (hash-routed pages, inline bilingual, mermaid), `mermaid-master` (diagram-first on light paper, inline SVG), `architecture` (one system diagram on slate, semantic colour), `sitemap-ia` (pages arguing a site structure, plus a clickable navigation prototype at two breakpoints), `project-website` (a project's public face: sticky nav, hash-routed pages, one shared footer, Google Fonts only). A seventh registry entry, `slide-design`, is a handoff rather than a template — see `D-013`. | [`../templates/templates.json`](../templates/templates.json) |
...
| `project-website` has no filled preview | `in-progress` | The skeleton was verified over `http://` — four pages reachable, both nav lists tracking, language switching both ways and restoring, deep links carrying language and page, unknown routes falling home, both themes, no console errors, no horizontal overflow at 320px or 1280px. What is missing is a filled preview like the other five have, which is also what the gallery thumbnail would be captured from. | [`../assets/tedandlisa-template-project-website.html`](../assets/tedandlisa-template-project-website.html), `D-019` |
