...
| ID | Decision | Status |
| --- | --- | --- |
| [`D-001`](#d-001-one-self-contained-html-file-per-deck) | One self-contained HTML file per deck | `accepted` |
| [`D-002`](#d-002-never-rewrite-the-deck-navigation-script-block) | Never rewrite the deck navigation script block | `accepted` |
| [`D-003`](#d-003-skills-are-agent-neutral-under-agentsskills) | Skills are agent-neutral under `.agents/skills/` | `accepted` |
| [`D-004`](#d-004-templates-are-named-for-how-they-read-not-for-their-subject) | Templates are named for how they read, not for their subject | `accepted` |
| [`D-005`](#d-005-project-context-is-updated-by-trigger-not-on-request) | Project context is updated by trigger, not on request | `accepted` |
| [`D-006`](#d-006-project-context-flows-one-way-into-this-repository) | Project Context flows one way into this repository | `accepted` |
| [`D-007`](#d-007-one-template-per-shape-not-one-template-with-switches) | One template per shape, not one template with switches | `accepted` |
| [`D-008`](#d-008-third-party-work-is-carried-by-the-lightest-mechanism-that-preserves-attribution) | Third-party work is carried by the lightest mechanism that preserves attribution | `accepted` |
| [`D-009`](#d-009-source-documents-never-enter-this-repository) | Source documents never enter this repository | `accepted` |
| [`D-010`](#d-010-english-and-korean-are-the-default-pair-for-inline-bilingual-templates) | English and Korean are the default pair for inline-bilingual templates | `accepted` |
| [`D-011`](#d-011-export-is-the-browsers-own-never-a-bundled-library) | Export is the browser's own, never a bundled library | `accepted` |
| [`D-012`](#d-012-the-intake-payload-outranks-the-command-line) | The intake payload outranks the command line | `accepted` |
| [`D-013`](#d-013-non-template-paths-live-in-the-same-registry-marked-external) | Non-template paths live in the same registry, marked `external` | `accepted` |
| [`D-014`](#d-014-previews-open-in-an-overlay-not-a-new-tab) | Previews open in an overlay, not a new tab | `accepted` |
| [`D-015`](#d-015-no-animation-library) | No animation library | `accepted` |
| [`D-016`](#d-016-the-readme-opens-with-the-start-prompt) | The README opens with the start prompt | `accepted` |
| [`D-017`](#d-017-intake-questions-can-be-scoped-to-a-template-not-only-to-a-kind) | Intake questions can be scoped to a template, not only to a kind | `accepted` |
| [`D-018`](#d-018-the-sitemap-ia-skeleton-ships-on-cdns-and-standalone-is-an-answer) | The `sitemap-ia` skeleton ships on CDNs, and standalone is an answer | `accepted` |
| [`D-019`](#d-019-project-website-keeps-the-sources-dark-palette-contrast-failure-included) | `project-website` keeps the source's dark palette, contrast failure included | `accepted` |
| [`D-020`](#d-020-standalone-html-only-the-pptx-output-is-retired) | Standalone HTML only — the `.pptx` output is retired | `accepted` |
| [`D-021`](#d-021-each-registry-carries-a-generated-index) | Each registry carries a generated index | `accepted` |
<!-- context-index:end -->

## D-022 — The web panel lists attachments, it does not inline them

`accepted`

The intake panel encodes attachments as base64 data URIs. That is right for the
local runner, which hands the payload to an agent as a file. It is wrong for the
website, where the reader copies the payload by hand: a 900KB image measured out
at a 1,230,087-character prompt, past what any chat box will accept, and the
existing 1.5MB warning never fired.

In web mode (`CTX.mode === "web"`) each reference is therefore sent as
`{ name, type, size, note }` with no `dataUri`, and the agent is told to ask the
user for the files. The runner keeps inlining. `references/intake-contract.md`
specifies both shapes and `SKILL.md` instructs agents to ask rather than guess
when `dataUri` is absent.

The alternative — inline under a size threshold — was considered and dropped:
it makes the payload shape depend on a number the reader cannot see, and the
failure it prevents is the same one the manifest prevents outright.
...
- Consequences: Claude Code reaches `/session-end` and `/project-context` through one-line pointers under `.claude/skills/`; Codex reads the managed block in `AGENTS.md`. A protocol with no pointer is reachable only through `CLAUDE.md` prose and the hook, which is how `project-context` went undiscoverable until `L-025`.
- Evidence: [`../AGENTS.md`](../AGENTS.md), [`../.claude/skills/session-end/SKILL.md`](../.claude/skills/session-end/SKILL.md), [`../.claude/skills/project-context/SKILL.md`](../.claude/skills/project-context/SKILL.md)
...
## D-021: Each registry carries a generated index

- Status: `accepted`
- Date: 2026-08-29
- Decision: `DECISIONS.md` and `LEARNINGS.md` open with a table of every entry —
  ID, its one-line statement, and its status or scope — generated from the
  entries themselves by `scripts/context_index.py`. The index is regenerated,
  never hand-edited.
- Rationale: These registries are read to answer one question: does anything
  here constrain what I am about to do? At 21 and 25 entries an agent still
  reads both end to end, spending context to find the two entries that matter.
  The growth curve is what makes that wrong, not today's size. Generating the
  index means it cannot disagree with the entries, which a hand-kept one would
  within a few commits — and a stale index is worse than none, because it is
  trusted.
- Consequences: Adding an entry means running the generator, and a hand-edited
  index is overwritten on the next run. Entry headings become load-bearing:
  they are the statement column, so a vague heading now degrades the index as
  well as the entry.
- Evidence: [`../scripts/context_index.py`](../scripts/context_index.py), [`DECISIONS.md`](DECISIONS.md), [`LEARNINGS.md`](LEARNINGS.md)
